...
def home_activities(request):
    ...
    events = Event.objects.filter(group__isnull=True)
...
def repost_post(request, post_id):
    ...
    Repost.objects.create(user=request.user, post=original)
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        return JsonResponse({'reposted': True, 'count': original.reposts.count()})
